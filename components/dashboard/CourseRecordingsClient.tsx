          ...
          filteredRecordings.map((rec) => {
            ...
            return (
              <div
                ...
                className="group bg-white border border-border rounded-[14px] p-4 flex flex-col md:flex-row items-center md:items-stretch gap-6 hover:border-primary/40 transition-colors shadow-sm w-full cursor-pointer"
              >
                {/* THUMBNAIL */}
                <div className="relative w-full md:w-[190px] h-[160px] md:h-auto md:min-h-[140px] shrink-0 bg-gray-900 rounded-[10px] overflow-hidden border border-gray-800 flex items-center justify-center">
                  {ytVid ? (
                    <img 
                      src={`https://img.youtube.com/vi/${ytVid}/hqdefault.jpg`}
                      alt="Thumbnail"
                      className="w-full h-full object-cover"
                      onError={(e) => {
                        (e.target as HTMLImageElement).src = `https://img.youtube.com/vi/${ytVid}/default.jpg`;
                      }}
                    />
                  ) : (
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity z-20 flex items-center justify-center">
                    <div className="w-10 h-10 rounded-full bg-white text-gray-900 flex items-center justify-center shadow-lg transform scale-90 group-hover:scale-100 transition-transform">
                ...
                {/* ACTION */}
                <div className="shrink-0 flex items-center md:justify-end md:w-[150px] w-full pt-2 md:pt-0" onClick={(e) => e.stopPropagation()}>
                  <WatchRecordingButton
                    sessionId={rec._id}
                    batchId={batchId}
                    onWatch={() => setSelectedRecording(rec)}
