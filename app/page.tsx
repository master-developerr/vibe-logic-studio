...
function ButtonLink({ children, href = "/sign-in?redirect_url=/checkout", variant = "primary" }: { children: ReactNode; href?: string; variant?: "primary" | "secondary" }) {
  return <a href={href} className={`button button-${variant}`}>{children}<span className="button-icon"><Icon name="arrow" size={15} /></span></a>;
}
  ...
  return (
    <main>
      <nav className="nav shell" aria-label="Primary navigation">
        ...
        <a href="/sign-in?redirect_url=/checkout" className="nav-cta">Join the cohort <Icon name="arrow" size={14} /></a>
      </nav>
      ...
      <section className="hero shell" id="top">
        <div className="hero-copy reveal">
          ...
          <div className="hero-actions"><ButtonLink>Enroll now</ButtonLink><ButtonLink href="#bootcamp" variant="secondary">Explore the bootcamp</ButtonLink></div>
      ...
      <section className="metrics"><div className="shell metrics-grid">{[["7", "Days to build"], ["7", "Real-world projects"], ["14+", "Hours, live with mentors"], ["₹999", "Launch offer"]].map(([value, label]) => <div key={label}><strong>{value}</strong><span>{label}</span></div>)}</div></section>
      ...
      <section className="shell bootcamp-section" id="bootcamp"><div className="section-heading"><div><p className="eyebrow amber">The flagship experience</p><h2>Become an AI developer<br />in just seven days.</h2></div><p>One focused, live cohort designed to turn curiosity into a working body of work.</p></div><div className="bootcamp-card"><div className="course-info"><div className="course-label">AI BUILD SPRINT · COHORT 01</div><h3>Build your first<br /><em>production-ready</em> web apps.</h3><div className="course-tags">{["Live classes", "7 projects", "Certificate", "Mentor support"].map(tag => <span key={tag}>{tag}</span>)}</div></div><div className="course-details"><div className="price"><small>Launch offer</small><strong>₹999</strong><span>one-time payment</span></div><ul>{["React, Next.js & Tailwind", "Convex + Clerk foundations", "AI-assisted development", "Deploy your work on Vercel"].map(item => <li key={item}><Icon name="check" size={16} />{item}</li>)}</ul><ButtonLink>Enroll now</ButtonLink><p className="course-footnote">Starts July 27 · recordings included</p></div></div></section>
