...
export default async function BuildSoftwareWithAI() {
  const course = await getCourseDetails("ai-build-sprint");
  ...
  return (
    <main>
      ...
      <nav className="nav shell" aria-label="Primary navigation">
        ...
        <Link href="/checkout?courseSlug=ai-build-sprint" className="nav-cta">Join the cohort <Icon name="arrow" size={14} /></Link>
      </nav>
      ...
      <section className="hero shell" id="top">
        <div className="hero-copy reveal">
          ...
          <div className="hero-actions"><ButtonLink href="/checkout?courseSlug=ai-build-sprint">Enroll now</ButtonLink><ButtonLink href="#bootcamp" variant="secondary">Explore the bootcamp</ButtonLink></div>
      ...
      <section className="shell bootcamp-section scroll-mt-[100px]" id="course-offer"><div id="bootcamp" className="scroll-mt-[100px]" /><div className="section-heading"><div><p className="eyebrow amber">The flagship experience</p><h2>Become an AI developer<br />in just seven days.</h2></div><p>One focused, live cohort designed to turn curiosity into a working body of work.</p></div><div className="bootcamp-card"><div className="course-info"><div className="course-label">AI BUILD SPRINT · COHORT 01</div><h3>Build your first<br /><em>production-ready</em> web apps.</h3><div className="course-tags">{["Live classes", "7 projects", "Certificate", "Mentor support"].map(tag => <span key={tag}>{tag}</span>)}</div></div><div className="course-details"><div className="price"><small>Launch offer</small><strong>{price}</strong><span>one-time payment</span></div><ul>{["React, Next.js & Tailwind", "Convex + Clerk foundations", "AI-assisted development", "Deploy your work on Vercel"].map(item => <li key={item}><Icon name="check" size={16} />{item}</li>)}</ul><ButtonLink href="/checkout?courseSlug=ai-build-sprint">Enroll now</ButtonLink><p className="course-footnote">Starts July 27 · recordings included</p></div></div></section>
